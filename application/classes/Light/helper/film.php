	...
	*/
	public static function parse(array $film, array $parse_options = array())
	{
		...
		$addition = array(
            ...
            'link_nd'					=> Light_Link::film('content', $film),
            'link_tag1'					=> Light_Link::film('tag', $film),
            ...
            'imagefan_url' 			=> self::get_image_url($film,'4',$config['img_server'], 1),
            'imagefan_thumb_url'             => self::get_image_url($film,'5',$config['img_server'],1),
            'imagefan_small_url'             => self::get_image_url($film,'6',$config['img_server'], 1),
            ...
            'short_pagetext_mobile' => Light_Helper_String::truncate_str($film['pagetext'], 130),
            'short_pagetext2_mobile' => Light_Helper_String::truncate_str($film['pagetext2'], 130),

            'mini_pagetext'			=> Light_Helper_String::truncate($film['pagetext'], 213),
            'mini_pagetext2'			=> Light_Helper_String::truncate($film['pagetext2'], 213),

            'date_format'			=> date('d-m',$film['last_update']),
            'liked_format'			=> number_format($film['liked_times']),
            'year'			=> $film['year'],
                        'status'			=> $film['status'],
'vote_format'			=> number_format($film['liked_vote']),
			'liked_timef_format'			=> number_format($film['liked_times_float']),
        ...
        if($parse_options['select_fields']){
			...
			$s_addition = Light_Helper_Array::array_find($film, $search);
			
			
			foreach($s_addition as $aitem)
			{
				$film = $aitem;
			}
		}
	 ...
	 * @return string
	*/
	public static function get_image_url(array $film, $is_thumbnail = FALSE, $local = FALSE, $wall = NULL){
        if($local){
            ...
            if(!empty($url)) {
                ...
                if ($is_thumbnail == 4) {
                    $url .= '.jpg';
                }elseif ($is_thumbnail == 5) {
                    $url .= '-thumb.jpg';
                }elseif ($is_thumbnail == 6) {
                    $url .= '-small.jpg';
                }
                $url = $domain.'data/images/films/'.$url;
            }else{
                $url = NULL;
            }
        }else{
	...
	*/
	public static function leech_image($url, $filename, $paths, $paths_temp, $wall)
	{
		...
		$leech = Light_Helper_File::leech_file($url, $temp_file = $paths_temp . $filename.'.jpg', $error);
		...
		if($leech)
		{
			...
			return self::_image_process($paths, $temp_file, $filename, TRUE, $wall);
		}
		return FALSE;
	}
	...
	*/
	protected static function _image_process($paths, $temp_file, $filename, $delete_temp = FALSE, $wall)
	{
		try
		{
			$config = Light_Config::get('config.film');
			...
			if($wall){
				...
				$image->resize($config['image_w_wall']);
				$image->save($data_dir . $filename_medium, $config['image_quality']);
				...
				$image->resize($config['image_w_thumb_wall']);
				//$image->crop($config['thumb_w'], $config['thumb_h'], 0); //crop from top-left
				$image->save($data_dir . $filename_thumb, $config['image_quality']);

				//small
				$filename_small = $new_filename.'-small'.$config['image_ext'];
				$image->resize($config['image_w_small_wall']);
				//$image->crop($config['thumb_w'], $config['thumb_h'], 0); //crop from top-left
				$image->save($data_dir . $filename_small, $config['image_quality']);
	...
	*/
	public static function rename_image_file($oldname, $newname, $film_id)
	{
        ...
        $folder_basic = $config['image_path'];
	...
	*/
	...
	public static function put_image_to_aaa($filename)
	{
	...
	public static function put_image_to_imgur($filename)
	{
        Light_Application::library('ChipVN/autoload');
