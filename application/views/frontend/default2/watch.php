...
<div class="black-block">
   <div class="block-movie container padding-z">
      <div id="media" class="block context-menu-one box menu-1">
         <div id="mediaplayer">
            <div id="player"></div>
            <c:if is="{$filminfo.setup_jwplayer}">
...
<!--      <c:if is="{$info_youtube}">-->
<!--                    <div style="margin-left:-5px">-->
<!--                        <span class="font_weight right-box-header star-icon"><span class="tbhd_light"></span>Phim liên quan</span>-->
<!--                        <div class="jcl-demo">-->
<!--                            <div class="custom-container moreItems6" style="position:relative">-->
<!--                                <div class="tbhd_prev" style="top:26px">-->
<!--                                    <a href="#" class="prev"></a>-->
<!--                                </div>-->
<!--                                <div class="carousel" style="width:100%">-->
<!--                                    <ul class="list-film">-->
<!--                                        <c:each from="$info_youtube" value="$youtube">-->
<!--                                            <li class="list-filmf" data-liked="{$film.liked_format}" data-views="{$film.views_format}" style="height:140px;width:169px">-->
<!--                                                <a href="/{$filminfo.link_watch}{$youtube.mahoa}" title="{$film.title}"><img style="height:140px;width:169px" src="{$youtube.snippet.thumbnails.high.url}" alt="{$film.title}"></a>-->
<!--                                            </li>-->
<!--                                        </c:each>-->
<!--                                    </ul>-->
<!--                                </div>-->
<!--                                <div class="tbhd_next" style="top:26px">-->
<!--                                    <a href="#" class="next"></a>-->
<!--                                </div>-->
<!--                                <div class="clear"></div>-->
<!--                            </div>-->
                   ...
                   <c:each from="$server_cache" key="$server_id" value="$server">
                       <div class="server">
                           <div class="list_carousel clear">
                               <div class="label">
                                   <i class="icon media-icon-server"></i>{$server.title} <!--<i class="icon media-server server-{$server.flag}"></i>-->
                               </div>
                               <ul class="mega-carousel episodelist">
                                   <c:each from="$episode_cache[$server_id]" value="$episode">
                                       <li>
                                           <a data-type="{$server.type}" data-episode-id="{$episode.episode_id}" href="{$episode.link}"
                                           <c:if is="{$admin} == '1' || {$admin} == '2'">
                                               title="Server: {$server.title} - Tập {$episode.name} - Link: {$episode.video_url}"
                                               <c:else />
                                               title="Tập {$episode.name} - {$server.title}"
                                           </c:if>
                                           <c:if is="{$episode.is_current}"> class="active"</c:if>
                                           >{$episode.name}</a>
                                       </li>
                                   </c:each>
                               </ul>
...
<div id="block-watch" class="body-wrap container">
   ...
   <div class="clear"></div>
   <div class="block-wrap padding rt-pa">
      <div class="block-col-left fontRoboto">
          <div class="padding-d row fix-row clear">

              <div id="profile-media" class="_4-u8 padding box-info">
                  <div class="col-lg-4 profile-media-img row col-media-img">
                          ...
                          <ul class="btn-block"><li class="item"><a data-target="#pop-trailer" class="btn btn-success" title="Download {$filminfo.title}"> Download </a></li></ul>

                  </div>
                  <!--end.profile-media-img-->
                  ...
                  <!--end.profile-media-content-->
                  <div class="profile-media-status col-content fix-row">
                      <div class="title_ascii hide">{$filminfo.title_ascii}</div>
                      <div class="category_id hide">{$filminfo.category_ids}</div>
                      <dl>
...
<!--            </div>-->
                    ...
                    <div class="ajax-load" data-dependencies="category">
