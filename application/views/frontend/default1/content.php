        ...
        <c:if is="$route_name">
            <div class="blocktitle breadcrumbs">
                ...
                <h2 class="item" itemscope itemtype="http://data-vocabulary.org/Breadcrumb">
                    <a href="{$url_canonical}" title="{$filminfo.title}" itemprop="url"><span title="{$filminfo.title}" itemprop="title"><c:if is="$route_name == 'content'">Nội dung </c:if>{$filminfo.title}</span></a>
                </h2>
            </div><!--/.breadcrumbs-->
        </c:if>
