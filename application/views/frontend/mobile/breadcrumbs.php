...
<c:if is="{$filminfo.type} == 1">
            <a  title="Phim lẻ" href="{$blocklinks.phim_le}">Phim lẻ</a>
            <c:else />
            <a  title="Phim bộ" href="{$blocklinks.phim_bo}">Phim bộ</a>
        </c:if>

                        

                        <a href="{$filminfo.country.link}">{$filminfo.country.name}</a><span>{$filminfo.title}</span></div>
